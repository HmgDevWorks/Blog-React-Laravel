...
function ProfileC() {
  ...
  useEffect(() => {
    const clientId = localStorage.getItem('clientId'); // Obtener el ID del cliente desde localStorage

    const fetchUser = async () => {
      try {
        const response = await userService.getOneUser({ clientId });
        // Asumiendo que la respuesta contiene los datos del usuario en las propiedades username, email y password
        setUsername(response.username);
        setEmail(response.email);
        setPassword(response.password);
        setCredenciales([response.username, response.email, response.password]);
      } catch (error) {
  ...
  //credenciales = [pipo, pipo@mail.com, password];
        ...
        <div className="card bg-base-100 w-[40vw] shadow-xl border-2 border-black">
          <div className="card-body flex flex-col items-center text-center">
            ...
            <label className="input input-bordered flex items-center gap-2">
              ...
              <input type="text" className="grow" placeholder={credenciales[0]} defaultValue={credenciales[0]} />
            </label>
            ...
            <label className="input input-bordered flex items-center gap-2">
              ...
              <input type="text" className="grow" placeholder={credenciales[1]} defaultValue={credenciales[1]} />
            </label>
            ...
            <label className="input input-bordered flex items-center gap-2">
              ...
              <input type="password" className="grow" placeholder={credenciales[2]} defaultValue={credenciales[2]} />
            </label>
